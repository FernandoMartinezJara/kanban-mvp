...
A local, Dockerized Project Management app: real (if lightweight) user accounts with account management, multiple Kanban boards per user, per-card priority/due-date tracking, a client-side card search, and an AI chat sidebar that can read/modify the active board via structured responses from an LLM. See `AGENTS.md` (root) for the original MVP business requirements and `docs/PLAN.md` for the phased build plan (Phase 2 covers the multi-user/multi-board expansion, Phase 3 covers card metadata/search/account management).
