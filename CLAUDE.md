...
A local, Dockerized Project Management app: real (if lightweight) user accounts with account management, multiple Kanban boards per user that owners can share with other registered users, per-card priority/due-date/assignee tracking, a client-side card search, and an AI chat sidebar that can read/modify the active board via structured responses from an LLM. See `AGENTS.md` (root) for the original MVP business requirements and `docs/PLAN.md` for the phased build plan (Phase 2 covers the multi-user/multi-board expansion, Phase 3 covers card metadata/search/account management, Phase 4 covers board sharing, Phase 5 covers card assignment).
...
- Auth: real accounts — `POST /api/auth/register` and `POST /api/auth/login` hash passwords with PBKDF2-HMAC-SHA256 (`db.hash_password`/`db.verify_password`, 200k iterations, random 16-byte salt per user) and issue a random per-session bearer token (`secrets.token_urlsafe(32)`) stored server-side in the JSON file's `sessions` map. `POST /api/auth/logout` deletes the session; `GET /api/auth/me` returns the current user; `POST /api/auth/change-password` re-verifies the current password before calling `db.set_user_password` (existing sessions are not invalidated — see `AccountMenu.tsx` for the frontend UI). Still no JWT/expiry — sessions live until logout or the DB file is reset.
...
- `frontend/src/lib/api.ts` — the only place that calls the backend (`login`, `register`, `logout`, `getCurrentUser`, `changePassword`, `listBoards`, `createBoard`, `getBoard`, `saveBoard`, `deleteBoard`, `shareBoard`, `unshareBoard`, `sendAIBoardQuery`). All authenticated calls attach `Authorization: Bearer <token>`.
...
### Users and boards (ownership and sharing model)
`backend/db.py` stores three top-level maps in the JSON file: `users` (id, username, `passwordHash`/`passwordSalt`, `createdAt`), `sessions` (token → `{userId, createdAt}`), and `boards` (id, `ownerId`, `memberIds`, `title`, `createdAt`, `columns`, `cards`). A board's owner can grant another registered user full read/write access via `POST /api/boards/{id}/share` (`{username}` → looked up with `db.find_user_by_username`, then `db.add_board_member`) and revoke it via `DELETE /api/boards/{id}/share/{memberId}` (`db.remove_board_member`) — both are owner-only actions (see below). There is exactly one access level: a member can view/edit/AI-query the board like the owner, but cannot delete it, share it, or manage its members.

Two board-lookup helpers exist and are **not interchangeable**: `db.get_owned_board` (owner only — used by `delete_board`, `share_board`, `unshare_board`) and `db.get_accessible_board` (owner or member — used by `get_board`, `update_board`, `ai_board`). Both return `None` for "board doesn't exist" *and* "caller has no access to it", and `backend/main.py` turns that into a 404 in both cases (deliberately, to avoid leaking board existence to outsiders). `db.list_boards_for_user` returns boards the user owns *or* is a member of. Any endpoint returning board data runs it through `db.board_response(data, board, viewerId)` first, which adds the viewer-relative fields the API/frontend need: `isOwner` (bool), `ownerUsername`, and `members` (resolved from `memberIds` to `{id, username}`) — these are computed per-request, not stored on the board, since "am I the owner" depends on who's asking. `POST /api/boards` creates a board with the standard five empty columns (`db.default_columns()`), no cards, and empty `memberIds`; there is no minimum-boards constraint, so a user can delete all of their boards (the frontend then shows `EmptyBoardsState`).

Frontend: `ShareBoardMenu.tsx` (rendered in `AppHeader.tsx` next to the board-title box) shows the share form + member list with per-member remove buttons when `board.isOwner`, or just a "Shared by {ownerUsername}" label otherwise. `BoardSwitcher.tsx` hides the per-board delete icon when `!board.isOwner` — a member can't delete a board they don't own, so the control isn't shown at all (backend still enforces this independently; the frontend hiding it is UX, not the security boundary).

### Board data model (must stay in sync across 3 places)
The board *content* shape (`columns: Column[]`, `cards: Record<string, Card>`, columns hold `cardIds: string[]`; a `Card` is `{id, title, details, priority, dueDate, assigneeId}` where `priority` is `"low" | "medium" | "high"` (default `"medium"`), `dueDate` is an ISO date string or `null`, and `assigneeId` is a user id or `null`) is duplicated in:
1. `frontend/src/lib/kanban.ts` (TS types — `BoardData` for content, `Board` adds `id`/`title`/`createdAt`/`ownerId`/`ownerUsername`/`isOwner`/`members`, `BoardSummary` is the list-view shape — plus `moveCard` logic, `initialData`, `cardMatchesQuery`, `isOverdue`, `boardCollaborators`)
2. `backend/schemas.py` (Pydantic models — `BoardContent`, `Board`, `BoardSummary`, `BoardCreateRequest`, `BoardUpdateRequest`, `Card.priority`/`Card.dueDate`/`Card.assigneeId` with defaults)
3. `backend/db.py` (`default_data()` seed board, `default_columns()` for new boards — same shape as JSON on disk)

`priority`/`dueDate`/`assigneeId` have Pydantic defaults, so boards written before these fields existed (or AI suggestions that omit them) still validate and serialize fine — no migration needed. Card priority, due date, and assignee are all editable inline on the card itself (`<select>`/`<input type="date">`/`<select>` inside `KanbanCard.tsx`, all with `onPointerDown={(e) => e.stopPropagation()}` so interacting with them doesn't start a drag) rather than through a separate edit form. `kanban.ts#isOverdue` flags a card whose `dueDate` has passed (red border/text on the due-date input); this is purely a display computation, not stored server-side. The board's search box (`KanbanBoard.tsx`, `cardMatchesQuery`) filters the rendered cards client-side by title/details substring match — it does not call the backend or mutate `board.cards`, and it also narrows the `SortableContext` items passed to `@dnd-kit` so a filtered-out card can't be a drag target.

`assigneeId` must be `null` or the id of the board's owner or one of its `memberIds` — `db.validate_assignees(board, content)` enforces this in `update_board` (→ 422 if violated) and in `ai_board` (an AI suggestion assigning a card to someone without board access is dropped, same as a referentially-broken board). The assignable-people list a card can be assigned to is `kanban.ts#boardCollaborators(board)` = the owner (`{id: board.ownerId, username: board.ownerUsername}`) plus `board.members` — this is why `schemas.Board` (unlike `BoardSummary`) exposes `ownerId`, not just `ownerUsername`: the frontend needs a real id to assign against, not just a display name.
