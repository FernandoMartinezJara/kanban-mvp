...
A local, Dockerized Project Management app: real (if lightweight) user accounts with account management, multiple Kanban boards per user that owners can share with other registered users, per-card priority/due-date tracking, a client-side card search, and an AI chat sidebar that can read/modify the active board via structured responses from an LLM. See `AGENTS.md` (root) for the original MVP business requirements and `docs/PLAN.md` for the phased build plan (Phase 2 covers the multi-user/multi-board expansion, Phase 3 covers card metadata/search/account management, Phase 4 covers board sharing).
...
The board *content* shape (`columns: Column[]`, `cards: Record<string, Card>`, columns hold `cardIds: string[]`; a `Card` is `{id, title, details, priority, dueDate}` where `priority` is `"low" | "medium" | "high"` (default `"medium"`) and `dueDate` is an ISO date string or `null`) is duplicated in:
1. `frontend/src/lib/kanban.ts` (TS types — `BoardData` for content, `Board` adds `id`/`title`/`createdAt`, `BoardSummary` is the list-view shape — plus `moveCard` logic, `initialData`, `cardMatchesQuery`, `isOverdue`)
2. `backend/schemas.py` (Pydantic models — `BoardContent`, `Board`, `BoardSummary`, `BoardCreateRequest`, `BoardUpdateRequest`, `Card.priority`/`Card.dueDate` with defaults)
3. `backend/db.py` (`default_data()` seed board, `default_columns()` for new boards — same shape as JSON on disk)

`priority`/`dueDate` have Pydantic defaults, so boards written before these fields existed (or AI suggestions that omit them) still validate and serialize fine — no migration needed. Card priority and due date are editable inline on the card itself (a `<select>` and `<input type="date">` inside `KanbanCard.tsx`, both with `onPointerDown={(e) => e.stopPropagation()}` so interacting with them doesn't start a drag) rather than through a separate edit form. `kanban.ts#isOverdue` flags a card whose `dueDate` has passed (red border/text on the due-date input); this is purely a display computation, not stored server-side. The board's search box (`KanbanBoard.tsx`, `cardMatchesQuery`) filters the rendered cards client-side by title/details substring match — it does not call the backend or mutate `board.cards`, and it also narrows the `SortableContext` items passed to `@dnd-kit` so a filtered-out card can't be a drag target.
